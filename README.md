...
The planner clusters visible image colors in display RGB to seed its initial palette. Pixels matching the bare background do not need thread at this stage, but **the background color is also considered as actual thread after the other paths exist**. For images with a broad tonal range and more than one allowed color, it reserves a dark thread for shadows and outlines. This endpoint uses the 2nd percentile of sampled brightness and allows for 2% background showing through, so dark details are not averaged into a dominant midtone. It compares short trial plans for the selected palette order, light-first and dark-first orders, then generates the full plan using the best trial order. Once the paths exist, it evaluates **all permutations of the used layers** (at most 120). This is a heuristic search, not proof of a globally optimal palette or winding.
...
Existing version 2 instructions retain their original rendering. New plans label their diagnostic errors with `stats.errorMetric: "detail-weighted-srgb-v1"`; these values cannot be compared directly with errors recorded by the old planner. Re-upload the source image to generate new paths with the improved planner. The line limit is a work budget, and stopping early only means the bounded search found no further useful moves; it does not establish a global maximum for a color.
